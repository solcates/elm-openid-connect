...
import App.Url as AUrl exposing (..)
import Array
import Browser exposing (Document)
import Browser.Events
import Browser.Navigation
import Html exposing (Attribute, Html, br, button, div, h1, p, text, ul)
...
import OpenIDConnect.Decode exposing (subDecoder)
...
main =
    Browser.application
        ...
        , view =
            view "Example App"
                { buttons =
                    [ viewSignInButton Google Authorize
                    ]
                , sn = sideNote
...
type Msg
    ...
    | StartPageMsg (App.Startpage.Msg Msg)
...
type alias Model =
    ...
    , is_drawer_open : Bool
    , navigateToUrl : Maybe AUrl.Url -- Used for animations
    , transition : Page.Transition
    , key : Browser.Navigation.Key
    , url : AUrl.Url
    , startpage : App.Startpage.Model Msg
    , headers : List Header
    , redirectUri : Url.Url
    , error : Maybe String
    , state : String
...
defaultModel : String -> Url.Url -> Browser.Navigation.Key -> Model
defaultModel state origin key =
    ...
    , key = key
    , url = AUrl.StartPage
    , startpage = App.Startpage.defaultModel
...
update msg model =
    ...
    case msg of
        ...
        UrlChanged url ->
            ( { model | url = AUrl.fromUrl url }, Cmd.none )
        ...
        SelectDrawerItem index ->
            let
                item =
                    Array.get index Page.drawerItems
            in
            case item of
                Just ( title, url ) ->
                    ( { model | navigateToUrl = Just url, transition = Page.Enter }, Cmd.none )
        ...
        Authorize config ->
            ...
            let
                auth =
                    OpenIDConnect.newAuth config.authorizationEndpoint model.redirectUri config.clientId config.scope
                        |> OpenIDConnect.withNonce model.state
        ...
        Navigate url ->
            ( { model
                | url = url
                , is_drawer_open =
                    if not model.useDismissibleDrawer then
                        False

                    else
                        model.is_drawer_open
                , transition =
            ...
            , Cmd.batch
                [ Browser.Navigation.pushUrl model.key (AUrl.toString url)
        ...
        SignOutRequested ->
            ( model
            , Browser.Navigation.load (Url.toString model.redirectUri)
            )

        StartPageMsg msg_ ->
            let
                ( startpage, effects ) =
                    App.Startpage.update StartPageMsg msg_ model.startpage
            in
...
view : String -> { onSignOut : Msg, buttons : List (Html Msg), sn : List (Html Msg) } -> Model -> Document Msg
view title { onSignOut, buttons, sn } model =
    let
        body =
...
view_ model =
    let
        bar =
            Page.topappbar Mdc "page-topappbar" model.mdc ToggleDrawer model.url
    ...
    case model.url of
        AUrl.StartPage ->
            App.Startpage.view StartPageMsg page model.startpage

        AUrl.Error404 requestedHash ->
            div
                []
